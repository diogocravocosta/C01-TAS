def samples_to_list(path, char_val):
    ...
    with open(path, 'r') as file:
        ...
        for line in file:
            ...
            if line.startswith('Sample'):
                ...
                for i in range(8):
                    latent_param = float(next(file).strip())
                    latent_parameters_lists[0][i].append(latent_param)
                    latent_parameters_lists[1][i].append(latent_param)
                ...
                if char_index == 0 and characteristic_value != -1:
                    characteristics_lists[0][char_index].append(float(characteristic_value))
                elif char_index == 1:
                    if characteristic_value != -1:
                        characteristics_lists[0][char_index].append(float(characteristic_value))
                    else:
                        for i in range(8):
                            latent_parameters_lists[0][i].pop                      
                else:
